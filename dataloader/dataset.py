...
# load CATS data
def load_cats_data(file_path, current_file):
    """ load current file from the list"""
    filename = file_path + 'left/' + current_file[0: len(current_file) - 1]
    ImageFile.LOAD_TRUNCATED_IMAGES = True
    left = Image.open(filename)
    # print(filename)
    filename = file_path + 'right/' + current_file[0: len(current_file) - 1] 
    right = Image.open(filename)
    filename = file_path + 'disp/' + current_file[0: len(current_file) - 1]
    disp_left = Image.open(filename)
...
class DatasetFromList(data.Dataset): 
    ...
    #    print self.file_list[index]
        if self.kitti: #load kitti dataset
            ...
            temp_data = load_cats_data(self.data_path, self.file_list[index])
